...
def evaluate(generateSolutions, testRoot, moduleDict, exceptionMap=ERROR_HINT_MAP,
    ...
    if questionToGrade == "q6":
        questions = []
        grades = grading.Grades(projectParams.PROJECT_NAME, questions,
                                gsOutput=gsOutput, edxOutput=edxOutput, muteOutput=muteOutput, hasQ6=True)
    ...
    if questionToGrade != None:
        grades = grading.Grades(projectParams.PROJECT_NAME, questions,
                                gsOutput=gsOutput, edxOutput=edxOutput, muteOutput=muteOutput, hasQ6=False)
...
if __name__ == '__main__':
    filenames = ["csp_test", "csp_test_2", "csp_test_3"]
    pathtoCSP = "./test_cases/q6/"
    ...
    if options.runTest != None:
        ...
        numCorrect = 0
        outputfile = pathtoCSP + "cspScore.txt"
        with open(outputfile, 'w') as outfile:
            for endPath in filenames:
                file = pathtoCSP + endPath
                with open(file + ".json") as infile, open(file + "_sol.json") as soljson:
                    data = json.load(infile)
                    sol = json.load(soljson)
                    CSP = csp.CSProblem(data)
                    for keys in sol['domains']:
                        sol['domains'][keys] = set(sol['domains'][keys])
                    try:
                        if(CSP.runAC3() == sol['domains']):
                            numCorrect = numCorrect + 1
                    except:
                        print('\nQuestion q6')
                        print('===========\n')
                        print('***FAIL: Terminated with a string exception.')
                        print('\n### Question 6: 0/3 ###\n')
                        
                        break
                    #print(numCorrect)
            outfile.write(str(numCorrect) + "/" + str(len(filenames)))
